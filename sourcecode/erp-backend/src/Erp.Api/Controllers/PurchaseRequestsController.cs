...
[ApiController]
[Authorize]
[Route("api/v1/purchasing/requests")]
public class PurchaseRequestsController(
    ErpDbContext db, RbacService rbac, WorkflowService wf, DocNumberingService numbering)
    : ControllerBase
{
    private const string Resource = "purchase-requests";

    private static PurchaseRequestOut ToDto(PurchaseRequest r) => new(
        r.Id, r.DocNo, r.DocDate, r.RequesterId, r.DepartmentId,
        r.Note, r.Status, r.StatusReason, r.CreatorId,
        r.Lines.Select(l => new PurchaseRequestLineOut(l.Id, l.ProductId, l.Quantity, l.Note)).ToList());
        ...
        [FromQuery] string? status, [FromQuery] int page = 1, [FromQuery] int size = 50)
    {
        ...
        var q = db.PurchaseRequests.Include(x => x.Lines).AsNoTracking().AsQueryable();
        ...
        return Ok(new PageResult<PurchaseRequestOut>(items.Select(ToDto).ToList(), total, page, size));
    }
    ...
    {
        if (await Denied("VIEW")) return Forbidden("VIEW");
        var r = await db.PurchaseRequests.Include(x => x.Lines).FirstOrDefaultAsync(x => x.Id == id);
        return r is null
            ? NotFound(new ApiError("NOT_FOUND", $"Yêu cầu mua hàng {id} không tồn tại"))
            : Ok(ToDto(r));
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] PurchaseRequestCreate body)
    {
        if (await Denied("CREATE")) return Forbidden("CREATE");

        await using var tx = await db.Database.BeginTransactionAsync();
        var r = new PurchaseRequest
        {
            DocNo = await numbering.NextAsync("PURCHASE_REQUEST"),
            DocDate = DateOnly.FromDateTime(DateTime.Today),
            RequesterId = body.RequesterId,
            DepartmentId = body.DepartmentId,
            Note = body.Note,
            CreatorId = RbacService.GetUserId(User),
            Lines = (body.Lines ?? new()).Select(l => new PurchaseRequestLine
            {
                ProductId = l.ProductId, Quantity = l.Quantity, Note = l.Note,
            }).ToList(),
        };
        db.PurchaseRequests.Add(r);
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update(long id, [FromBody] PurchaseRequestUpdate body)
    {
        if (await Denied("UPDATE")) return Forbidden("UPDATE");
        var r = await db.PurchaseRequests.Include(x => x.Lines).FirstOrDefaultAsync(x => x.Id == id);
        if (r is null) return NotFound(new ApiError("NOT_FOUND", $"Yêu cầu mua hàng {id} không tồn tại"));
        if (r.Status != "DRAFT")
    ...
    public async Task<IActionResult> AddLine(long id, [FromBody] PurchaseRequestLineIn body)
    {
        if (await Denied("UPDATE")) return Forbidden("UPDATE");
        var r = await db.PurchaseRequests.FirstOrDefaultAsync(x => x.Id == id);
        if (r is null) return NotFound(new ApiError("NOT_FOUND", $"Yêu cầu mua hàng {id} không tồn tại"));
        if (r.Status != "DRAFT")
            return Conflict(new ApiError("WF_LOCKED", $"Trạng thái {r.Status}, không sửa được"));
        var line = new PurchaseRequestLine
        {
            RequestId = id, ProductId = body.ProductId, Quantity = body.Quantity, Note = body.Note,
        };
        db.PurchaseRequestLines.Add(line);
        await db.SaveChangesAsync();
        return StatusCode(201, new PurchaseRequestLineOut(line.Id, line.ProductId, line.Quantity, line.Note));
    }
    ...
    public async Task<IActionResult> DeleteLine(long id, long lineId)
    {
        ...
        var line = await db.PurchaseRequestLines
            .FirstOrDefaultAsync(x => x.Id == lineId && x.RequestId == id);
    ...
    public async Task<IActionResult> Action(long id, string actionName, [FromBody] WfActionRequest? body)
    {
        ...
        if (body?.Reason is not null) r.StatusReason = body.Reason;
        await db.SaveChangesAsync();
        return Ok(ToDto(r));
    }
}